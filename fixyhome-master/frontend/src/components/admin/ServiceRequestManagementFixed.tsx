...
export default function ServiceRequestManagementFixed({ token }: ServiceRequestManagementProps) {
  ...
  const fetchRequests = async () => {
    try {
      const url = filterStatus === "ALL" 
        ? 'http://localhost:8080/api/admin/service-requests-admin'
        : `http://localhost:8080/api/admin/service-requests-admin?status=${filterStatus}`;
      ...
      if (response.ok) {
        ...
        const transformedRequests = data.map((request: any) => ({
          ...request,
          client: {
            id: request.client?.id || 1,
            firstName: request.client?.firstName || 'Client',
            lastName: request.client?.lastName || 'Test',
            email: request.client?.email || 'client@test.com',
            phone: request.client?.phone || '0000000000'
          },
          artisan: request.artisan ? {
            id: request.artisan.id,
            firstName: request.artisan.firstName,
            lastName: request.artisan.lastName,
            email: request.artisan.email
          } : undefined
        }));
  ...
  const updateRequestStatus = async (requestId: number, newStatus: string) => {
    try {
      const response = await fetch(`http://localhost:8080/api/admin/service-requests-admin/${requestId}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const deleteRequest = async (requestId: number) => {
    try {
      const response = await fetch(`http://localhost:8080/api/admin/service-requests-admin/${requestId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
